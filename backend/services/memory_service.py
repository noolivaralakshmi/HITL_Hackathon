...
def update_memory(memory_id: str, **kwargs) -> dict:
    ...
    json_fields = ['detection_reasons', 'reasoning', 'missing_info', 'guardrail_flags', 'groundedness']
...
def approve_memory(memory_id: str, user_id: str) -> dict:
    """Approve a memory record and index it for semantic search."""
    ...
    # Index for semantic search (async-safe, non-blocking on failure)
    memory = get_memory(memory_id)
    try:
        from backend.services.embedding_service import index_memory
        index_memory(memory_id, memory)
    except Exception:
        pass  # Semantic index failure shouldn't block approval
