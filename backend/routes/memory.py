...
from backend.services.ai_service import analyze_documents, detect_missing_info, verify_groundedness
...
def generate_memory(req: GenerateMemoryRequest):
    ...
    # Groundedness verification - check AI reasoning against source documents
    groundedness_result = None
    try:
        groundedness_result = verify_groundedness(reasoning, documents_text_safe)
    except Exception:
        groundedness_result = {
            "claims": [],
            "groundedness_score": {"supported": 0, "partially_supported": 0, "unsupported": 0, "total": 0, "percentage": 0},
            "summary": "Groundedness verification unavailable.",
            "critical_gaps": [],
        }
    ...
    log_action(db, memory_id, None, "AI_DRAFT", risk_level=risk_level, details={
        ...
        "groundedness_score": groundedness_result.get("groundedness_score", {}).get("percentage", 0) if groundedness_result else 0,
        "unsupported_claims": len(groundedness_result.get("critical_gaps", [])) if groundedness_result else 0,
...
@router.post("/{memory_id}/verify-groundedness")
def verify_memory_groundedness(memory_id: str):
    """Run groundedness verification on a memory's reasoning against its source documents.

    This can be called independently to re-verify after edits, or to get
    detailed citation information for review.
    """
    memory = get_memory(memory_id)
    if not memory:
        raise HTTPException(status_code=404, detail="Memory not found")
    ...
    if not reasoning:
        raise HTTPException(status_code=400, detail="Memory has no reasoning to verify")
    ...
    if not docs:
        raise HTTPException(status_code=400, detail="No source documents found for this memory")
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"Content blocked during verification: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Groundedness verification failed: {str(e)}")

    # Update the memory with fresh groundedness result
    update_memory(memory_id, groundedness=groundedness_result)
